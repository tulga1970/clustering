...
const cluster = (n, distanceMatrix, max_d) => {
  ...
  for (let i = 0; i < n; i++) {
    for (let j = i + 1; j < n; j++) {
      pq.push([i, j, D[condensendIndex(n, i, j)]]);
    }
  }
  ...
  for (let k = 0; k < n - 1; k++) {
    ...
    while (true) {
      ...
      if (chain_length > 1 && y && y == cluster_chain[chain_length - 2]) {
        break;
      }
  ...
  // labels.map((_, idx) => (res[idx] = root(labels, idx)));
  return labels.map((_, idx) => (res[idx] = root(labels, idx)));
};
